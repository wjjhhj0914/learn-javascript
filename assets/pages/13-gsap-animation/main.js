...
(() => {
  ...
  function tweenAnimate() {
    const greenTween = gsap.to('.green', {
      ...
      onRepeat: () => {
        console.log('2배속');
        greenTween.timeScale(2)
      }, // repeat이 되면 timeScale 2배속
...
(() => {
  ...
  const tl = gsap.timeline({
    ...
    yoyo: true,
    defaults: {
      x: -200, duration: 0.4, opacity: 0, rotation: 270
    }
  })
    .from('.blue', {  })
    .from('.yellow', {  })
  ...
  // GSDevTools 인스턴스 생성
  GSDevTools.create({ animation: tl, persist: false, paused: false, timeScale: 2, loop: true });
});